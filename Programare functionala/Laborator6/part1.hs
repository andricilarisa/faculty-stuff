...
skipWhiteSpace "" = ""
skipWhiteSpace  (x:xs) | x == ' ' = skipWhiteSpace (xs)
                         | otherwise = (x:xs)
...
getInteger "" = ("", "")
getInteger (x:xs) | isDigit x == True = (x:fst(getInteger xs), snd(getInteger xs))
...
tokenNext (x:xs)
                 | x == '+' = Just (TSuma, xs)
                 | x == '*' = Just (TProdus, xs)
                 | isDigit x == True = Just (TInt (fst (getInteger (x:xs))), (snd(getInteger (x:xs) )))
                 | otherwise = Nothing 
...
--tokenize [] = Just []
--tokenize (x:xs) | nows xs == "" = Just []
--               | (isNothing thisToken) = Nothing
--               | (isNothing nextToken) = Nothing
--                | otherwise = Just ([fst jt] ++ jn)
--                where nows x = skipWhiteSpace (x:xs)
--                      thisToken = tokenNext (nows xs)
--                      jt = jval thisToken
--                      nextToken = tokenize (snd jt)
--                      jn = jval nextToken
...
tokenize s = if skipWhiteSpace s == "" then Just []
            ...
            Nothing -> Nothing
            Just (token, rest) -> case tokenize rest of
                                Nothing->Nothing
                                Just tl -> Just (token:tl) 