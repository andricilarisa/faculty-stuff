...
sumaMaybe (Just x) Nothing = Nothing
sumaMaybe  Nothing (Just y) = Nothing
sumaMaybe (Just x) (Just y) = Just(x + y) 

produsMaybe (Just x) Nothing = Nothing
produsMaybe Nothing (Just y) = Nothing
...
evalp (Produs e1 e2) val = produsMaybe (evalp e1 val) (evalp e2 val)
