...
executaInstr valuatie (Bucla e p) = case (evalp e valuatie) of
                                        ...
                                        otherwise -> case (executaInstr valuatie (Bucla e p)) of 
                                                        Nothing->Nothing
                                                        Just valuatie' -> executaProgram valuatie' p

evalProgram :: Program -> Maybe Int
evalProgram prog = case (executaProgram empty prog) of
                      Just valuatie -> valuatie "result"
                      Nothing       -> Nothing
-- evalProgram [(Atrib "result" (Numar 10)), Cond (Diferenta (Var "result") (Numar 8)) [Atrib "result" (Numar 1)][Atrib "result" (Numar 0)]] -- exemplul
--exemplu pentru while
-- un program exemplu

--prog2 :: Program
--prog2 = [Atrib "n" (Numar 10), Atrib "s" (Numar 0), Bucla (Var "n") [Atrib "s" (Suma (Var "s") (Var "n")), Atrib "n" (Diferenta (Var "n") (Numar 1))],Atrib "result" (Var "s")]
--exemplu pentru bulca for :D
