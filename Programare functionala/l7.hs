...
Produs (Var "x")(Suma (Numar 2)(Var "y"))
-}
...
eval (Produs (Var "x")(Suma (Numar 2)(Var "y"))) val1 
eval (Produs (Suma (Var "x")(Numar 10)) (Numar 2)) val1
...
sumaMaybe Nothing (Just m) = Nothing
sumaMaybe (Just n) Nothing = Nothing

inmultireMaybe :: Maybe Int -> Maybe Int -> Maybe Int
inmultireMaybe (Just n) (Just m) = Just (n * m)
inmultireMaybe Nothing (Just m) = Nothing
inmultireMaybe (Just n) Nothing = Nothing

evalp :: Exp -> ValuationPar -> Maybe Int
evalp (Numar x) _ = Just x
evalp (Var v) val = val v
evalp (Suma e1 e2) val = sumaMaybe (evalp e1 val) (evalp e2 val)
evalp (Produs e1 e2) val = inmultireMaybe (evalp e1 val) (evalp e2 val)